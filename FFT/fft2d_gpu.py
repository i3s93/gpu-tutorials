...
parser = argparse.ArgumentParser(description="A 2D Poisson solver based on the FFT (GPU).")
...
def exact_solution(x,y):
	...
	"""
	return cp.sin(10*x)*cp.cos(5*y)

def f(x,y):
	"""
	The source term in the 2D Poisson equation.
	"""
	return 125*cp.sin(10*x)*cp.cos(5*y)

def poisson_2d_fft(f, dx, dy, plan):
	"""
	...
	K2 = cp.outer(k**2, cp.ones_like(l))
	L2 = cp.outer(cp.ones_like(k), l**2)
	...
	u = cp.real( cufft.ifft2(u_hat, plan=plan) )
...
def main():
	...
	X_grid, Y_grid = cp.meshgrid(x,y,indexing="ij")

	# Generate the source data
	source = f(X_grid, Y_grid)

	# Get the solution for the error analysis
	u = poisson_2d_fft(source, dx, dy, plan)

	# Measure the error with the analytical solution
	exact = exact_solution(X_grid,Y_grid)
	error1 = dx*dy*cp.sum(cp.abs(u - exact))
	error2 = cp.sqrt( dx*dy*cp.sum(cp.abs(u - exact)**2) )
	...
	print(benchmark(poisson_2d_fft, (source, dx, dy, plan), n_repeat=reps))
